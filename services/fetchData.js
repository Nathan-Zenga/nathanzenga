...
const openConnection = async () => prod && connection.readyState === 0 ? await connect(process.env.DB, connectOptions) : null;
...
export const getDesignWork = async (body = {}) => {
  await openConnection();
  const designs = JSON.stringify(await model("Design").find(body).lean().sort({ index: 1 }).exec());
